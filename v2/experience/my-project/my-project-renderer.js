...
(function (global) {
    ...
    function renderPerformanceReport(state) {
        ...
        var outputAction = state.performanceReportOutput ? '<a class="primary-button connector-download-link" href="' + e(state.performanceReportOutput.downloadUrl) + '" download="' + e(state.performanceReportOutput.downloadFileName) + '">출력용 HTML 다운로드</a>' : '<button class="primary-button" data-action="prepare-performance-report-output">기관 양식 출력 준비</button>';
        var draftPanel = state.performanceReportDraft ? '<section class="report-draft-panel"><div><span class="section-kicker">생성 완료</span><h2>' + e(state.performanceReportDraft.title) + '</h2><p>Operation ' + e(state.performanceReportDraft.operationVersion) + ' · 단위사업 실적 ' + state.performanceReportDraft.unitProjectResults.length + '건 · 보고서 구성 ' + state.performanceReportDraft.sections.length + '개</p></div><div class="report-draft-actions"><button class="secondary-button" data-action="download-performance-report">초안 JSON</button>' + outputAction + '</div></section>' : '';
